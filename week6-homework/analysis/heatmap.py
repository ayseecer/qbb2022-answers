...
def filter(raw_data, start_bin, end_bin): 
    ...
    mask = numpy.where((raw_data['F1'] >= start_bin) & (raw_data['F2'] < end_bin))
    filter_data = raw_data[mask]
...
def main():
    ...
    ax[0].imshow(-smooth_removed_matrix_data1, vmax=vmax, vmin=vmin, cmap='magma')                           
    ax[0].axis('off')
    ax[1].imshow(-smooth_removed_matrix_data2, vmax=vmax, vmin=vmin, cmap='magma')
    ax[1].axis('off')
    ax[2].imshow(matrix3, cmap='seismic', norm=colors.CenteredNorm())
    ax[2].axis('off')
